...
def scatter_usage(df, server='dali'):
    ...
    plt.ylabel('Size [GB]')
    plt.title('%s: %s(%s)/%sTB; \n %s(%s)/%s files'%(server, np.round(df['total_tb'].sum(), decimals=2), used_tb_dict[server], limit_tb_dict[server], df['total_n'].sum(),used_files_dict[server], limit_files_dict[server]))
    
    mask = ((df['total_n'] > alarm_files_dict[server]) | (df['total_tb'] > alarm_tb_dict[server])) & (df['name']!='xenonnt') & (df['name']!='xenon1t')

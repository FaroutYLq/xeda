...
class Submit(object):
    ...
    def _submit_single(self, loop_index, loop_item):
        ...
        jobstring = "python /home/yuanlq/software/xeda/xeda.py -t 500GB -o '/project2/lgrandi/yuanlq/shared/disk_usage/' -d '[xenonnt, xenon1t]' -s %s"%(loop_item)
        print(jobstring)

        # Modify here for the log name
        utilix.batchq.submit_job(
            jobstring, log='/home/yuanlq/.tmp_job_submission/scan_%s.log'%(loop_item.split('/')[1]), 
            partition='dali', qos='dali',
            account='pi-lgrandi', jobname=jobname,
            delete_file=True, dry_run=False, mem_per_cpu=25000,

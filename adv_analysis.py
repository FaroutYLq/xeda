...
DB_DTYPE = [
    (('datetime of the scan',
      'time'), datetime),
    (('total size in TB unit',
      'total_tb'), np.float32),
    (('total file counts', 
      'total_n'), np.int32),
    (('rawdata size in TB unit',
      'rawdata_tb'), np.float32),
    (('rawdata_file counts', 
      'rawdata_n'), np.int32),
    (('records size in TB unit',
      'records_tb'), np.float32),
    (('records file counts', 
      'records_n'), np.int32),
    (('peaks size in TB unit',
      'peaks_tb'), np.float32),
    (('peaks file counts', 
      'peaks_n'), np.int32),
    (('root size in TB unit',
      'root_tb'), np.float32),
    (('root file counts', 
      'root_n'), np.int32),
    (('pickle size in TB unit',
      'pickle_tb'), np.float32),
    (('pickle file counts', 
      'pickle_n'), np.int32),
    (('job files size in TB unit',
      'jobs_tb'), np.float32),
    (('job file counts', 
      'jobs_n'), np.int32),
    (('figures size in TB unit',
      'figures_tb'), np.float32),
    (('figures file counts', 
      'figures_n'), np.int32),
    (('zips size in TB unit',
      'zips_tb'), np.float32),
    (('zips file counts', 
      'zips_n'), np.int32),
    (('hdf size in TB unit',
...
def find_file_list(scan_within='/dali/lgrandi/', directory='/project2/lgrandi/yuanlq/shared/disk_usage/'):
    assert (scan_within[0] == '/' and scan_within[-1] == '/'), 'Please check format of scan_within. Good example: /dali/lgrandi/'
...
def make_db(scan_within='/dali/lgrandi/', directory='/project2/lgrandi/yuanlq/shared/disk_usage/'):
    files_oi = find_file_list(scan_within, directory)
    db = {}

    for file_oi in files_oi:
        time = datetime(int(file_oi[-12:-8]), clean_up_date(file_oi[-8:-6]), clean_up_date(file_oi[-6:-4]))
